...
import { ShieldAlert, Eye, EyeOff, Lock, Network, UserPlus } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-surface-950 flex flex-col justify-center py-12 sm:px-6 lg:px-8 font-sans selection:bg-brand-500/30 text-surface-50 relative overflow-hidden">
      ...
      <div className="absolute inset-0 z-0">
        <div className="absolute top-1/4 right-1/4 w-96 h-96 bg-brand-500/10 rounded-full blur-[120px]"></div>
        <div className="absolute bottom-1/4 left-1/4 w-96 h-96 bg-pink-500/10 rounded-full blur-[120px]"></div>
      ...
      <div className="sm:mx-auto sm:w-full sm:max-w-md relative z-10 animate-in slide-in-from-bottom-4 duration-700">
        ...
        <p className="mt-2 text-center text-sm text-brand-400 font-mono flex items-center justify-center gap-2">
          <Lock size={14}/> CLEARANCE REGISTRATION
        </p>
      </div>
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md relative z-10">
        <div className="glass-panel py-8 px-4 sm:rounded-3xl sm:px-10 border border-brand-500/30 relative overflow-hidden group">
          ...
          <form className="space-y-6" onSubmit={handleSignup}>
            <div>
              <label htmlFor="name" className="block text-xs font-mono text-surface-400 uppercase tracking-wider mb-2">
                Full Legal Name
              </label>
              <div className="mt-1">
                <input
            ...
            <div className="flex items-center justify-between mt-4">
               <p className="text-xs text-surface-500 font-mono flex items-center gap-1">
                <ShieldAlert size={12} className="text-yellow-500"/> Level 3 Required
              </p>
              <NavLink to="/login" className="text-xs text-brand-400 hover:text-brand-300 font-mono transition-colors border-b border-brand-500/30 hover:border-brand-400 pb-0.5">
                RETURN TO LOGIN
              </NavLink>
            </div>
